...
readarray -t ki < <(find legacy -mindepth 3 -maxdepth 3 -name '*.keyboard_info')
...
for kb in "${ki[@]}"; do
  ...
  if [[ ! -f "/c/temp/ki/$base.keyboard_info" ]]; then
    echo "$COLOR_RED failed to download for $base $COLOR_RESET"
    continue
  fi

  cp "/c/temp/ki/$base.keyboard_info" "$kb"
